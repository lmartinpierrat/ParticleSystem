...
(const glm::vec3 &position, const glm::vec3 &rotation, const glm::vec3 &scale, unsigned int maxParticles) :
...
(int count)
{
    ...
    for (int i = 0 ; i < count ; ++i)
    {
        ...
        this->_particles.push_back(new Particle(position, rotation, scale, this->_startLifetime.getValue(), this->_startSpeed.getValue(), this->_startColor.getValue(), this->_gravityMultiplier, force, this->_colorValueOverLifetime, this->_sizeOverLifetime, this->_rotationOverLifetime));
    }
}
